...
class SectionSelection extends StatefulWidget {
  ...
  final Map<String, dynamic> professor; // ✅ professor passed through
...
class _SectionSelectionState extends State<SectionSelection> {
  ...
  Future<void> _loadSections() async {
    ...
    if (subjectId != null) {
      filtered = all.where((a) => a['subject_id'].toString() == subjectId.toString()).toList();
    } else {
      filtered = all.where((a) =>
          a['subject_name'].toString().toLowerCase() ==
          widget.subjectName.toLowerCase()).toList();
    }
    setState(() { _assignments = filtered; _loading = false; });
  }
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: const Text('SENTRY',
              style: TextStyle(fontFamily: 'sans', fontSize: 28,
                  fontStyle: FontStyle.italic, color: Color(0xFF1E3A8A),
        ...
        body: Column(
          children: [
            ...
            Text('Select a Section', style: TextStyle(fontFamily: 'sans', fontSize: 16, color: Colors.grey[600])),
            const SizedBox(height: 4),
            Text(widget.subjectName,
                style: const TextStyle(fontFamily: 'sans', fontSize: 22, fontWeight: FontWeight.bold, color: Colors.black)),
              ...
              child: _loading
                  ...
                  : _assignments.isEmpty
                      ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                          Icon(Icons.groups_rounded, size: 56, color: Colors.grey[300]),
                          const SizedBox(height: 12),
                          Text('No sections found for\n${widget.subjectName}',
                              textAlign: TextAlign.center, style: TextStyle(color: Colors.grey[500], fontSize: 14)),
                          const SizedBox(height: 8),
                          Text('Ask the admin to assign sections.', style: TextStyle(color: Colors.grey[400], fontSize: 12)),
                        ]))
                      : SingleChildScrollView(
                          ...
                          child: Container(
                            ...
                            child: GridView.builder(
                              ...
                              itemBuilder: (context, index) => _buildSectionCard(_assignments[index]),
                            ),
                          ),
                        ),
            ),
                ...
                child: ElevatedButton(
                  onPressed: _selectedAssignment != null
                      ? () => Navigator.push(context, MaterialPageRoute(
                            builder: (_) => StudentScanner(
                              subjectSectionId: _selectedAssignment!['subject_section_id'],
                              sectionName: _selectedAssignment!['section_name'],
                              professor: widget.professor, // ✅ pass professor
                            ),
                          ))
                      : null,
  ...
  Widget _buildSectionCard(Map<String, dynamic> assignment) {
    final isSelected = _selectedAssignment?['subject_section_id'] == assignment['subject_section_id'];
    return GestureDetector(
      ...
      child: Container(
        decoration: BoxDecoration(
          ...
          boxShadow: isSelected ? [BoxShadow(color: Colors.blue.withOpacity(0.3), blurRadius: 8, spreadRadius: 2)] : null,
        ),
        child: Stack(children: [
          Positioned(bottom: 12, right: 12,
              child: Icon(Icons.groups_rounded, color: Colors.white.withOpacity(0.5), size: 40)),
          Padding(
            padding: const EdgeInsets.all(12),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(assignment['section_name'] ?? '',
                  style: const TextStyle(fontFamily: 'sans', fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
              const SizedBox(height: 4),
              if (assignment['course'] != null)
                Text(assignment['course'], style: TextStyle(fontSize: 11, color: Colors.white.withOpacity(0.85))),
              if (assignment['professor_name'] != null)
                Text(assignment['professor_name'],
