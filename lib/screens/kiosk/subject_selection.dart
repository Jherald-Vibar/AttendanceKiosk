...
class _SubjectSelectionState extends State<SubjectSelection> {
  // Groups subjects by year_level field from DB
  ...
  // Maps year_level value to a display label
  ...
  // Cycles through card gradient colors
  ...
  Widget build(BuildContext context) {
    ...
    // Flat list of all subjects for gradient index tracking
    int cardIndex = 0;
    ...
    return Scaffold(
        ...
        actions: [
          Padding(
            ...
            child: CircleAvatar(
              ...
              child: Text(
                (widget.professor['full_name'] as String? ?? 'P')
                    .substring(0, 1)
                    .toUpperCase(),
      ...
      body: Column(
        children: [
          ...
          // Professor name
          Text(
            'Prof. ${widget.professor['full_name'] ?? ''}',
            style: const TextStyle(
              fontFamily: 'sans',
              fontSize: 13,
          ...
          // Next button (disabled — selection handled by card tap)
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 24),
            child: SizedBox(
              ...
              child: ElevatedButton(
                onPressed: null, // Card tap handles navigation
        ...
        Navigator.push(
          ...
          MaterialPageRoute(
            builder: (context) => SectionSelection(
              subjectName: title,
              subject: subject, // ✅ pass full subject map for ID-based filtering
            ),
          ),
        );
      },
